...
# print(f"[import] chanlun.config, fun: {time.time() - _import_start:.2f}s")

from chanlun.base import Market
# print(f"[import] chanlun.base: {time.time() - _import_start:.2f}s")
...
# print(f"[import] chanlun.cl_utils: {time.time() - _import_start:.2f}s")

from chanlun.config import get_data_path
# print(f"[import] chanlun.config.get_data_path: {time.time() - _import_start:.2f}s")

from chanlun.db import db
# print(f"[import] chanlun.db: {time.time() - _import_start:.2f}s")

from chanlun.exchange import get_exchange
# print(f"[import] chanlun.exchange: {time.time() - _import_start:.2f}s")
...
# print(f"[import] chanlun.tools.cache: {time.time() - _import_start:.2f}s")

from chanlun.zixuan import ZiXuan
# print(f"[import] chanlun.zixuan: {time.time() - _import_start:.2f}s")

from .alert_tasks import AlertTasks
# print(f"[import] .alert_tasks: {time.time() - _import_start:.2f}s")

from .other_tasks import OtherTasks
# print(f"[import] .other_tasks: {time.time() - _import_start:.2f}s")

from .xuangu_tasks import XuanguTasks
# print(f"[import] .xuangu_tasks: {time.time() - _import_start:.2f}s")

import requests
# print(f"[import] requests: {time.time() - _import_start:.2f}s")

# _create_app_start_time = time.time()

# def log_cl_app(msg):
#     """记录 cl_app 启动日志到文件"""
#     try:
#         elapsed = time.time() - _create_app_start_time
#         log_line = f"[cl_app {elapsed:.2f}s] {msg}\n"
#         with open("D:/txt/333.txt", "a", encoding="utf-8") as f:
#             f.write(log_line)
#     except:
#         pass
...
#         log_line = f"[函数耗时] {func.__name__}: {elapsed:.4f}秒\n"
#         print(log_line.strip())
#         try:
#             with open("D:/txt/history", "a", encoding="utf-8") as f:
#                 f.write(log_line)
#         except:
#             pass
#         return result
...
def create_app(test_config=None):
    ...
    # log_cl_app("开始加载市场配置...")
    # 使用硬编码配置，避免启动时初始化所有交易所导致启动慢
    # 各个市场支持的时间周期（硬编码）
    market_frequencys = {
        "a": ["y", "m", "w", "d", "60m", "30m", "15m", "5m", "1m"],
        "hk": ["d", "60m", "30m", "15m", "10m", "5m", "1m"],
        "fx": ["d", "60m", "30m", "15m", "5m", "1m"],
        "us": ["d", "60m", "30m", "15m", "5m", "1m"],
        "futures": ["d", "60m", "30m", "15m", "5m", "1m"],
        "ny_futures": ["d", "60m", "30m", "15m", "5m", "1m"],
        "currency": ["d", "6h", "4h", "3h", "2h", "1h", "30m", "15m", "5m", "1m"],
        "currency_spot": ["d", "6h", "4h", "3h", "2h", "1h", "30m", "15m", "5m", "1m"],
    }
    # log_cl_app("market_frequencys 加载完成")
    ...
    market_default_codes = {
        ...
        "ny_futures": "NYMEX.CL",
        "currency": "BTCUSDT",
        "currency_spot": "BTCUSDT",
    }
    # log_cl_app("market_default_codes 加载完成")
    ...
    _alert_tasks.run()
    # log_cl_app("AlertTasks 初始化完成")

    _xuangu_tasks = XuanguTasks(scheduler)
    # log_cl_app("XuanguTasks 初始化完成")

    _other_tasks = OtherTasks(scheduler)
    # log_cl_app("OtherTasks 初始化完成")
    ...
    # log_cl_app(f"Flask 应用创建完成，总耗时: {time.time() - _create_app_start_time:.2f}秒")
    ...
    def tv_symbols():
        ...
        _log_file = "/home/shengge/桌面/hanshu2.txt"
        def _log(msg):
            try:
                elapsed = time.time() - _func_start
                with open(_log_file, "a", encoding="utf-8") as f:
                    f.write(f"[+{elapsed:.3f}s] {msg}\n")
            except:
                pass
        ...
        code: str = symbol[1]
        _log(f"解析参数: market={market}, code={code}")

        ex = get_exchange(Market(market))
        _log("获取交易所对象完成")

        # 从 Redis 缓存中读取股票信息
        cache_key = f"stock_info:{market}:{code}"
        stocks = cache.get(cache_key)
        _log(f"股票信息缓存读取: {'命中' if stocks else '未命中'}")
        if stocks is None:
            _log("开始获取股票信息")
            stocks = ex.stock_info(code)
            _log(f"获取股票信息完成: {stocks}")
            # 缓存数据，设置过期时间为 1 小时
            cache.set(cache_key, stocks, expire=3600)
            _log("股票信息缓存写入完成")
        ...
        if market == "a":
            _log("A股市场，获取板块信息")
            # 从 Redis 缓存中读取股票板块信息
            cache_key = f"stock_plate:{market}:{code}"
            gnbk = cache.get(cache_key)
            _log(f"板块信息缓存读取: {'命中' if gnbk else '未命中'}")
            if gnbk is None:
                try:
                    _log("开始获取板块信息")
                    ...
                    _log(f"获取板块信息完成")
                    # 缓存数据，设置过期时间为 1 小时
                    cache.set(cache_key, gnbk, expire=3600)
                    _log("板块信息缓存写入完成")
            ...
            _log(f"板块拼接完成: sector={sector}, industry={industry}")
        ...
        _log(f"函数执行完成，总耗时: {time.time() - _func_start:.3f}秒")
        return info
    ...
    def tv_search():
        ...
        if all_stocks is None:
            ...
            # 缓存数据，设置过期时间为 1 小时
            cache.set(cache_key, all_stocks, expire=3600)
    ...
    def tv_history():
        ...
        """
        _func_start = time.time()
        _log_file = "/home/shengge/桌面/hanshu.txt"
        def _log(msg):
            try:
                elapsed = time.time() - _func_start
                with open(_log_file, "a", encoding="utf-8") as f:
        ...
        _log(f"获取参数: symbol={symbol}, resolution={resolution}, firstDataRequest={firstDataRequest}")
    ...
    # 获取仓位设置
    @app.route("/position-setting", methods=["GET"])
    @login_required
    # @measure_time
    def get_position_setting():
        try:
            # 从PythonProject1项目获取仓位设置
            response = requests.get(f"{config.PYTHON_PROJECT_URL}/api/position-setting")
    ...
    def ticks():
        ...
        # 从 Redis 缓存中读取股票涨跌幅数据
        stock_ticks = {}
        missing_codes = []

        for code in codes:
            cache_key = f"stock_tick:{market}:{code}"
            tick_data = cache.get(cache_key)
            if tick_data is not None:
                ...
                class TickDict:
                    ...
                    @property
                    def last(self):
                        return self.__dict__.get('last', 0)
                    @property
                    def rate(self):
                        return self.__dict__.get('rate', 0)
                stock_ticks[code] = TickDict(tick_data)
            else:
                missing_codes.append(code)

        # 对缓存中没有的数据，从交易所获取
        if missing_codes:
            new_ticks = ex.ticks(missing_codes)
            for code, tick in new_ticks.items():
                stock_ticks[code] = tick
                # 缓存数据，设置过期时间为 1 分钟（因为涨跌幅数据更新频繁）
                # 转换对象为可序列化的字典
                tick_dict = {}
                for key, value in tick.__dict__.items():
                    if hasattr(value, 'isoformat'):
                        ...
                        tick_dict[key] = value
                cache.set(f"stock_tick:{market}:{code}", tick_dict, expire=60)
        ...
        try:
            ...
            cursor.execute("""
                ...
                LEFT JOIN stock_change sc
                    ON sf.stock_code COLLATE utf8mb4_unicode_ci = sc.stock_code COLLATE utf8mb4_unicode_ci
                ORDER BY
                    CASE WHEN sc.change_rate IS NULL THEN 1 ELSE 0 END,
                    sc.change_rate DESC,
                    sf.follow_time DESC
            """)
            ...
            for stock in stocks:
                if stock.get("change_rate") is not None:
                    stock["change_rate"] = float(stock["change_rate"])
