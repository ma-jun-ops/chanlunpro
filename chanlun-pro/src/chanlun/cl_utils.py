...
def cl_data_to_tv_chart(
    ...
    """
    # K线
    src_klines = cd.get_klines()
    if len(src_klines) == 0:
        return None
    kline_ts = [fun.datetime_to_int(k.date) for k in src_klines]
    kline_hs = [k.h for k in src_klines]
    kline_ls = [k.l for k in src_klines]
    kline_os = [k.o for k in src_klines]
    kline_cs = [k.c for k in src_klines]
    kline_vs = [k.a for k in src_klines]

    if to_frequency is not None:
        # 将数据转换成指定的周期数据
        klines = pd.DataFrame(
            {
                "date": [k.date for k in src_klines],
                "high": kline_hs,
                "low": kline_ls,
                "open": kline_os,
                "close": kline_cs,
