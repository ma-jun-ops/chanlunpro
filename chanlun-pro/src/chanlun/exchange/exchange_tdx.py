...
from chanlun.tools import cache, get_cache_queue
import pandas as pd
import pytz
# 初始化缓存队列
...
class ExchangeTDX(Exchange):
    ...
    def __init__(self):
        ...
        try:
            # 选择最优的服务器，并保存到缓存队列中
            self.connect_info = cache_queue.get("tdx_connect_ip")
    ...
    def reset_tdx_ip(self):
        ...
        # 使用缓存队列异步保存
        cache_queue.set("tdx_connect_ip", connect_info, expire=3600, async_=True)
    ...
    def to_tdx_code(self, code):
        ...
        # 使用原始的 for_sh/for_sz 分类规则，避免调用 all_stocks() 导致的 GBK 解码问题
        if market == 2:
            _type = "stock_cn"
        else:
            # 根据代码前缀判断类型
    ...
    def klines(
        ...
        # 首先尝试从缓存队列获取数据
        cache_key = f"stock:klines:{code}:{frequency}:{args['fq']}"
        cached_data = cache_queue.get(cache_key)
        if cached_data is not None:

            try:
                # 反序列化缓存数据
                ks = pd.DataFrame(cached_data)
                ks['date'] = pd.to_datetime(ks['date'])
                if ks['date'].dt.tz is None:
                    ks['date'] = ks['date'].dt.tz_localize(self.tz)
                else:
                    ks['date'] = ks['date'].dt.tz_convert(self.tz)
                return ks
            except Exception as e:
                print(f"缓存数据解析失败: {e}")
        ...
        try:
            ...
            try:
                # 序列化数据
                cache_data = ks.to_dict('records')
                cache_queue.set(cache_key, cache_data, expire=3600)
            except Exception as e:
                print(f"缓存数据失败: {e}")
    ...
    def ticks(self, codes: List[str]) -> Dict[str, Tick]:
            ...
            if cached_data is not None:
                try:
                    ...
                    ticks[code] = Tick(
                        ...
                        buy1=cached_data["buy1"],
                        sell1=cached_data["sell1"],
                        low=cached_data["low"],
        ...
        # 剩余需要从API获取的代码
        remaining_codes = [code for code in codes if code not in cached_codes]
        if not remaining_codes:
            return ticks
        
        query_stocks = []
        stock_types = {}
        for _c in remaining_codes:
        ...
        with client.connect(self.connect_info["ip"], self.connect_info["port"]):
            ...
            for _q in quotes:
                ...
                try:
                    ...
                    cache_queue.set(cache_key, cache_data, expire=300)  # 实时数据缓存5分钟
                except Exception as e:
                    print(f"缓存数据失败: {e}")
            print(datetime.now().timestamp() - start,"xxxxxxxxxx",file="D:/txt/log.txt")
